...
public interface IEnergyConductor extends IEnergyConnector {
	...
	public static int getIdentityFromTile(TileEntity te) {
		return getIdentityFromPos(te.getPos().getX(), te.getPos().getY(), te.getPos().getZ());
	}
	...
	public default List<int[]> getConnectionPoints() {
		...
		for(ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
			int newX = tile.getPos().getX() + dir.offsetX;
			int newY = tile.getPos().getY() + dir.offsetY;
			int newZ = tile.getPos().getZ() + dir.offsetZ;
